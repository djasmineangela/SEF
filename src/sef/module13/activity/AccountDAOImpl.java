...
import static sef.module13.activity.AccountDAOException.ERROR_FIND_ID;
import static sef.module13.activity.AccountDAOException.ERROR_INSERT_ACCOUNT;
import static sef.module13.activity.AccountDAOException.ERROR_FIND_NAME;

public class AccountDAOImpl implements AccountDAO {

	private static final String SELECT_ALL_ACCOUNTS = "SELECT id, first_name, last_name, e_mail FROM account";
	private static final String SELECT_ACCOUNTS_BY_NAME = SELECT_ALL_ACCOUNTS
			+ " WHERE LOWER(first_name) LIKE LOWER(?) AND LOWER(last_name) LIKE LOWER(?) ORDER BY id";
	...
	private AccountImpl toAccount(ResultSet row) throws SQLException {
		AccountImpl account = new AccountImpl(row.getInt(ID_COLUMN), row.getString(FIRST_NAME_COLUMN),
				row.getString(LAST_NAME_COLUMN), row.getString(EMAIL_COLUMN));
		return account;
	}
